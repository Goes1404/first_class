...
/* ─── Página ───────────────────────────────────────────────────────────── */
export const CollectionTab: React.FC<{ group: CollectionGroup }> = ({ group }) => {
  const { usePageVisit } = useAnalytics();
  usePageVisit(group.analyticsKey);
  ...
  const items = useMemo(
    () => (products ?? []).filter((p) => group.matches(p.category)),
    [products, group],
  );
  ...
  const byBrand = useMemo(
    () => (brand === ALL ? items : items.filter((p) => p.brand?.trim() === brand)),
    [items, brand],
  );
  ...
  const pills = useMemo(() => {
    const names = [...new Set(items.map((p) => p.category).filter(Boolean))].sort((a, b) =>
  ...
  return (
    <div className="min-h-screen bg-white pb-28">
      <SEO title={group.seoTitle} description={group.tagline} />
      ...
      <header className="sticky top-0 z-30 bg-white">
        <div className="mx-auto max-w-5xl px-4 h-16 grid grid-cols-[44px_1fr_44px] items-center">
          ...
          <div className="flex flex-col items-center leading-none">
            <span className="text-[17px] font-bold text-slate-900">{group.title}</span>
          ...
          {!isLoading && !isError && shown.length === 0 && (
            <div className="mt-3 rounded-2xl border border-slate-200 p-8 flex flex-col items-center text-center gap-2">
              ...
              <p className="text-sm font-semibold text-slate-700">
                {items.length === 0 ? group.emptyTitle : 'Nada nesse filtro'}
              </p>
